...
You can then use the `.omenad-meend-hindi` class on any element to use the font:

```html
<code class="omenad-meend-hindi">g  @pp g  p</code>
```
...
```html
<table class="composition meend-hindi">
  <tbody>
    <tr>
      <td></td>
      <td></td>
...
table.composition {
  ...
  &.meend-hindi td > code {
    white-space: pre;
    font-family: 'omenad_meend_hindi';
  }
}
```
